...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
    ...
    func testSaveAndLoadEntity() {
        ...
        let results = managedObjectContext?.executeFetchRequest(request, error: nil) as! [Bowtie]
        let myBowtie = results[0]
        
        println("保存的领结是 \(myBowtie.name), \(myBowtie.lastWorn)")
    }
